...
class User(models.Model):
    ...
    def toDict(self):
        return {'id': self.id, 'username': self.username, 'nickname': self.nickname,
                ...
                'create_at': self.create_at.datetime.strftime('%Y-%m-%d %H:%M:%S'),
                'update_at': self.update_at.datetime.strftime('%Y-%m-%d %H:%M:%S')}
...
class Member(models.Model):
    ...
    status=models.IntegerField(default=1)
    create_at=models.DateTimeField(datetime.now)
    update_at=models.DateTimeField(datetime.now)

    def toDict(self):
        return {'id': self.id, 'nickname': self.nickname, 'avatar': self.avatar, 'mobile': self.mobile,
                'status': self.status, 'create_at': self.create_at.datetime.strftime('%Y-%m-%d %H:%M:%S'),
                'update_at': self.update_at.datetime.strftime('%Y-%m-%d %H:%M:%S')}
